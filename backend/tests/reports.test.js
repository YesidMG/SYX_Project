...
describe("Reports API", () => {
  ...
  afterAll(async () => {
    ...
    const complaintsRes = await request(app).get(`/api/complaints?entity_id=${testEntity.id}`);
    for (const c of complaintsRes.body) {
      await request(app).delete(`/api/complaints/${c.id}`);
    }
    // Borrar la entidad
    await request(app).delete(`/api/entities/${testEntity.id}`);
    await pool.end();
  });
