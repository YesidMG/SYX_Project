...
class ComplaintService {
    ...
    async getComplaintById(id) {
        ...
        if (!complaint) throw { status: HTTP_STATUS.NOT_FOUND, message: ERROR_MESSAGES.COMPLAINT_NOT_FOUND };
        return complaint;
    }

    async getComplaintsByEntity(entityId) {
        if (isNaN(entityId)) throw { status:HTTP_STATUS.BAD_REQUEST, message: ERROR_MESSAGES.INVALID_ENTITY_ID };
        return await complaintRepo.findByEntityId(entityId);
    }

    async createComplaint(data) {
        if (!data.title || typeof data.title !== FIELD_TYPES.STRING || data.title.trim() === EMPTY_STRING) {
            throw { status: HTTP_STATUS.BAD_REQUEST, message: ERROR_MESSAGES.TITLE_REQUIRED };
        }
        if (!data.description || typeof data.description !== FIELD_TYPES.STRING || data.description.trim() === EMPTY_STRING) {
